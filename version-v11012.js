...
function freezeSidebarVersion(){
  ...
  if(legacy){legacy.id='wdSideVersionV11012';legacy.textContent=`v${VERSION}`;}
  const previous=document.getElementById('wdSideVersionV11011');
  if(previous){previous.id='wdSideVersionV11012';previous.textContent=`v${VERSION}`;}
  const fixed=document.getElementById('wdSideVersionV11012');
  if(fixed&&fixed.textContent!==`v${VERSION}`)fixed.textContent=`v${VERSION}`;
}
