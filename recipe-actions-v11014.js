...
function ensureActions(){
  ...
  const box=document.createElement('div');box.className='wd-recipe-actions';
  box.innerHTML='<button class="btn ghost" id="wdAddRecipeShopping" type="button">加入采购清单</button><button class="btn primary" id="wdEatTomorrow" type="button">明天就吃它</button>';
  body.appendChild(box);$('#wdAddRecipeShopping').onclick=addToShopping;$('#wdEatTomorrow').onclick=addToTomorrow;
}
async function addToShopping(){
  const rows=$$('#recipeDialogContent .ingredient');
  const items=rows.map((el,i)=>({id:`manual:recipe:${Date.now()}:${i}`,name:$('b',el)?.textContent.trim()||'',qtyText:$('.meal-meta',el)?.textContent.trim()||'1份',category:'其他'})).filter(x=>x.name);
  ...
  toast(`已加入 ${items.length} 项采购食材`);
  $('#recipeDialog')?.close();location.hash='#shopping';
}
async function addToTomorrow(){
  ...
  try{
    if(src&&src.startsWith(location.origin)) imageUrl=src;
      ...
      const response=await fetch(src);if(response.ok){const blob=await response.blob();const ext=(blob.type.split('/')[1]||'jpg').replace('jpeg','jpg');imagePath=`${user.id}/${groupId}-${crypto.randomUUID()}.${ext}`;const up=await sb.storage.from('dish-images').upload(imagePath,blob,{contentType:blob.type||'image/jpeg',upsert:false});if(up.error)imagePath=null;}
    }
    const {error}=await sb.from('site_dishes').insert({name,description,category,image_path:imagePath,image_url:imageUrl,is_active:true,created_by:user.id,group_id:groupId});
    if(error)throw error;
    toast('已加入当前小饭桌的明日候选');$('#recipeDialog')?.close();location.hash='#tomorrow';
  }catch(error){toast(error?.message||'加入候选失败');}
}
function init(){const root=$('#recipeDialogContent');if(!root)return;new MutationObserver(()=>ensureActions()).observe(root,{childList:true,subtree:true});ensureActions();}
