...
const flags = [{key:'重点',icon:'★'},{key:'考试',icon:'✓'},{key:'没听懂',icon:'?'},{key:'例子',icon:'◇'}]
const state = {
...
function setSync(busy=false){state.saving=Math.max(0,state.saving+(busy?1:-1));$('syncPill').textContent=state.saving?'☁ 同步中…':'☁ 已同步'}
...
function render(){
  $('recordCount').textContent=`${state.entries.length} 条记录`
  ...
  $('recordLabel').textContent=state.isListening?'暂停听课':'开始听课'
  ...
  $('originalStream').innerHTML=orig+(state.interim?`<article class="speech-card interim"><div class="meta"><span>正在识别</span></div><p>${esc(state.interim)}</p></article>`:'') || empty('点击下方“开始听课”，老师的讲话会显示在这里。')
  const trans=state.entries.map(e=>`<article class="speech-card translated"><div class="meta"><span>${esc(e.time)}</span></div><p class="${e.translation?.startsWith('⚠')?'error':''}">${esc(e.translation||'翻译中…')}</p><div class="flag-row">${flags.map(f=>`<button data-entry="${esc(e.id)}" data-flag="${esc(f.key)}" class="${e.flags.includes(f.key)?'selected':''}">${f.icon} ${esc(f.key)}</button>`).join('')}</div></article>`).join('')
  $('translationStream').innerHTML=trans||empty('识别完成后会自动翻译，并和原文按时间对应。')
  $('notesArea').innerHTML=state.notes?`<pre>${esc(state.notes)}</pre>`:empty('下课后点击“生成 AI 课堂笔记”。')
...
async function ensureCloudSession(){
  ...
  setSync(true)
  const {data,error}=await supabase.from('classflow_sessions').insert({title:$('classTitle').value||'未命名课堂',source_language:$('sourceLanguage').value,target_language:'zh-CN'}).select('id').single()
  setSync(false)
  if(error){setStatus(`云端创建失败：${error.message}`);throw error}
...
async function saveSegment(entry){
  try{
    ...
    const {data,error}=await supabase.from('classflow_segments').insert({session_id:sessionId,seq:entry.seq,source_text:entry.original,translation:entry.translation==='翻译中…'?'':entry.translation,flags:entry.flags}).select('id').single()
    setSync(false); if(error)throw error; entry.cloudId=data.id; persistLocal()
  }catch(err){setSync(false);setStatus(`字幕已保存在本机，云同步失败：${err.message||err}`)}
}
...
async function translateEntry(entry){
  ...
  if(state.aiReady===false){entry.translation='⚠ AI 翻译后端尚未配置';render();return}
  try{
  ...
  }catch(err){entry.translation=`⚠ ${err.message||'翻译失败'}`}
  render()
}

async function addFinal(text){
  const entry={id:crypto.randomUUID(),cloudId:null,seq:state.entries.length+1,time:time(),original:text.trim(),translation:'翻译中…',flags:[]}
  state.entries.push(entry);state.interim='';render();await saveSegment(entry);translateEntry(entry)
}

function createRecognition(){
  const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition
  if(!Recognition){setStatus('此浏览器暂不支持网页实时语音识别。请优先使用最新版 Chrome / Edge / Safari。');return null}
  ...
  r.onerror=(ev)=>{if(ev.error==='not-allowed'){state.shouldRestart=false;state.isListening=false;setStatus('麦克风权限被拒绝，请在浏览器网站权限中允许麦克风');render()}else if(ev.error!=='no-speech'){setStatus(`语音识别异常：${ev.error}`)}}
...
async function start(){
  ...
  try{r.start();state.isListening=true;setStatus('正在听课、翻译并自动保存');render()}catch{setStatus('无法启动麦克风，请刷新页面后重试')}
}
function stop(){state.shouldRestart=false;state.recognition?.stop();state.isListening=false;state.interim='';setStatus('已暂停，课堂记录已保存');render()}
...
async function generateNotes(){
  ...
  const b=$('generateNotes');b.disabled=true;b.textContent='正在整理…';setStatus('正在生成课堂笔记')
  try{
    ...
    if(error)throw error;setStatus('AI 课堂笔记已生成并保存')
  }catch(err){setSync(false);setStatus(err.message||'生成笔记失败')}
  finally{b.textContent='生成 AI 课堂笔记';render()}
}
...
function exportWord(){const rows=state.entries.map(e=>`<div class="entry"><h3>${esc(e.time)}${e.flags.length?' · '+esc(e.flags.join(' / ')):''}</h3><p><b>原文：</b>${esc(e.original)}</p><p><b>翻译：</b>${esc(e.translation)}</p></div>`).join('');const html=`<!doctype html><html><head><meta charset="utf-8"><style>body{font-family:Arial,'Microsoft YaHei',sans-serif;line-height:1.7;padding:28px}h1{font-size:24px}.entry{margin:0 0 22px;padding-bottom:14px;border-bottom:1px solid #ddd}h3{font-size:14px;color:#666}p{font-size:12pt}.notes{white-space:pre-wrap}</style></head><body><h1>${esc($('classTitle').value)}</h1>${rows}${state.notes?`<h1>AI 课堂笔记</h1><div class="notes">${esc(state.notes)}</div>`:''}</body></html>`;blobDownload('\ufeff'+html,'application/msword','doc')}

async function newSession(){
  stop();state.sessionId=null;state.entries=[];state.notes='';state.interim='';clearLocal();$('classTitle').value='新课堂';setStatus('已新建课堂，点击“开始听课”');render();await loadHistory()
}
...
async function loadHistory(){
  ...
  if(error)return
  state.history=data||[]
  $('historyList').innerHTML=state.history.length?state.history.map(s=>`<button class="history-item" data-id="${s.id}"><strong>${esc(s.title)}</strong><span>${dateTime(s.started_at)} · ${esc(s.source_language)}</span></button>`).join(''):empty('还没有云端课堂记录。')
  document.querySelectorAll('.history-item').forEach(b=>b.onclick=()=>loadSession(b.dataset.id))
}

async function loadSession(id){
  stop();setStatus('正在读取云端课堂…')
  ...
  if(e1||e2){setStatus(`读取失败：${(e1||e2).message}`);return}
  ...
  closeHistory();setStatus('已载入云端课堂记录');render()
}
...
async function saveSessionMeta(){
  ...
  const payload={title:$('classTitle').value||'未命名课堂',source_language:$('sourceLanguage').value,updated_at:new Date().toISOString()}
  setSync(true);const {error}=await supabase.from('classflow_sessions').update(payload).eq('id',state.sessionId);setSync(false);if(error)setStatus(`云同步失败：${error.message}`)
}
...
async function enterApp(session){
  ...
  setStatus('云端已连接');render();await Promise.all([loadHistory(),checkAI()])
}

$('authForm').onsubmit=async(e)=>{e.preventDefault();showAuthMessage('正在登录…');try{await signIn($('authEmail').value.trim(),$('authPassword').value);showAuthMessage('登录成功')}catch(err){showAuthMessage(err.message||'登录失败',true)}}
$('signupButton').onclick=async()=>{showAuthMessage('正在注册…');try{const d=await signUp($('authEmail').value.trim(),$('authPassword').value);showAuthMessage(d.session?'注册并登录成功':'注册成功，请检查邮箱完成确认后再登录')}catch(err){showAuthMessage(err.message||'注册失败',true)}}
...
document.addEventListener('visibilitychange',()=>{if(document.hidden&&state.isListening)setStatus('页面在后台，部分手机会暂停麦克风；回到页面后会自动尝试继续')})
