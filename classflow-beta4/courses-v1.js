...
function patchBeta4Labels(){
  document.title='ClassFlow Beta 4.0 · v1'
  const status=$('status')
  if(status&&/BETA 3|Beta 3/i.test(status.textContent))status.textContent='BETA 4.0 · v1 · Course → Lesson / 课程分层'
...
async function boot(){
  ...
  installToolbar();patchBeta4Labels()
  let tries=0
  const timer=setInterval(()=>{
    tries++;installMainAddLesson();installObserver();patchBeta4Labels()
